"""
Summarize Pass 8 CommonVoice partial-label pretraining results and collapse modes.

Expected input naming convention:
  results/eval_<metric>_pass8_<condition>.csv
...
FILENAME_RE = re.compile(r"eval_(emotion|wer|mos|novelty)_pass8_(.+)\.csv$")
...
def parse_args():
    ...
    ap.add_argument(
        "--summary-out",
        default="results/eval_commonvoice_partial_label_summary_pass8.csv",
    )
    ap.add_argument(
        "--collapse-out",
        default="results/eval_commonvoice_partial_label_collapse_pass8.csv",
    )
...
def discover_metric_files(results_dir):
    ...
    for path in sorted(Path(results_dir).glob("eval_*_pass8_*.csv")):
    ...
    if not grouped:
        raise SystemExit(f"No Pass 8 result CSVs found under {results_dir}")
    return grouped
