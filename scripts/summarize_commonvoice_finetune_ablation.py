"""
Summarize Pass 5 CommonVoice finetune-ablation results and collapse modes.

Expected input naming convention:
  results/eval_<metric>_pass5_<condition>.csv
...
FILENAME_RE = re.compile(r"eval_(emotion|wer|mos|novelty)_pass5_(.+)\.csv$")
...
def parse_args():
    ...
    ap.add_argument(
        "--results-dir",
        default="results",
        help="Directory containing eval_*_pass5_*.csv files (default: results)",
    )
    ap.add_argument(
        "--summary-out",
        default="results/eval_commonvoice_finetune_summary_pass5.csv",
        help="Condition-level summary CSV output",
    )
    ap.add_argument(
        "--collapse-out",
        default="results/eval_commonvoice_finetune_collapse_pass5.csv",
...
def discover_metric_files(results_dir):
    ...
    for path in sorted(Path(results_dir).glob("eval_*_pass5_*.csv")):
    ...
    if not grouped:
        raise SystemExit(f"No Pass 5 result CSVs found under {results_dir}")
    return grouped
...
def main():
    ...
    print("Pass 5 CommonVoice finetune summary")
    print("=" * 88)
    for row in summary_rows:
        print(
            f"{row['condition']:28s} "
            ...
            f"Δrecall(cv500)={row['delta_recall_vs_cv500'] or 'n/a':>7s} "
            f"Δnovelty(cv500)={row['delta_novelty_vs_cv500'] or 'n/a':>7s}"
        )
