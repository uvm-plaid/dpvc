"""
Summarize Pass 4 ablation results and track collapse modes.

Expected input naming convention:
  results/eval_<metric>_pass4_<condition>.csv
...
FILENAME_RE = re.compile(r"eval_(emotion|wer|mos|novelty)_pass4_(.+)\.csv$")
...
def parse_args():
    ...
    ap.add_argument(
        "--results-dir",
        default="results",
        help="Directory containing eval_*_pass4_*.csv files (default: results)",
    )
    ap.add_argument(
        "--summary-out",
        default="results/eval_ablation_summary_pass4.csv",
        help="Condition-level summary CSV output",
    )
    ap.add_argument(
        "--collapse-out",
        default="results/eval_ablation_collapse_pass4.csv",
...
def discover_metric_files(results_dir):
    ...
    for path in sorted(Path(results_dir).glob("eval_*_pass4_*.csv")):
    ...
    if not grouped:
        raise SystemExit(f"No Pass 4 result CSVs found under {results_dir}")
    return grouped
...
def main():
    ...
    print("Pass 4 ablation summary")
    print("=" * 72)
    for row in summary_rows:
        print(
            f"{row['condition']:24s} "
            f"styles={row['styles_count']:>2s} "
            ...
            f"collapses={row['files_with_any_collapse']}"
        )
