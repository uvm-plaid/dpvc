"""
Summarize Pass 7 CommonVoice rich-objective results and collapse modes.

Expected input naming convention:
  results/eval_<metric>_pass7_<condition>.csv
...
FILENAME_RE = re.compile(r"eval_(emotion|wer|mos|novelty)_pass7_(.+)\.csv$")
...
def parse_args():
    ...
    ap.add_argument(
        "--results-dir",
        default="results",
        help="Directory containing eval_*_pass7_*.csv files (default: results)",
    )
    ap.add_argument(
        "--summary-out",
        default="results/eval_commonvoice_rich_objectives_summary_pass7.csv",
        help="Condition-level summary CSV output",
    )
    ap.add_argument(
        "--collapse-out",
        default="results/eval_commonvoice_rich_objectives_collapse_pass7.csv",
...
def discover_metric_files(results_dir):
    ...
    for path in sorted(Path(results_dir).glob("eval_*_pass7_*.csv")):
    ...
    if not grouped:
        raise SystemExit(f"No Pass 7 result CSVs found under {results_dir}")
    return grouped
...
def add_delta_columns(summary_rows):
    ...
    for row in summary_rows:
        ...
        row["delta_recall_vs_combined"] = safe_delta(as_float(row, "emotion_recall"), combined_recall)
...
def main():
    ...
    for condition, metric_paths in grouped.items():
        summary, rows = summarize_condition(condition, metric_paths)
        summaries.append(summary)
        collapse_rows.extend(rows)

    order_index = {name: idx for idx, name in enumerate(SUMMARY_ORDER)}
    summaries.sort(key=lambda row: (order_index.get(row["condition"], 999), row["condition"]))
    collapse_rows.sort(key=lambda row: (
        order_index.get(row["condition"], 999),
        row["speaker"],
        row["style"],
    ))

    add_delta_columns(summaries)
